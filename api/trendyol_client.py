...
class TrendyolClient:
    ...
    def get_orders_to_process(self, max_pages: int = 5) -> Dict[str, Any]:
        """
        Get orders that are ready to be processed.
        Fetches orders from BOTH "Picking" and "Invoiced" statuses across multiple pages.
        
        Args:
            max_pages: Maximum number of pages to fetch per status (default: 5)
        ...
        try:
            ...
            for status in statuses:
                ...
                # Fetch up to max_pages for this status
                while page < max_pages:
                    self.logger.info(f"Fetching {status} orders - page {page}")
                    response = self.get_orders(status=status, size=100, page=page)
                    
                    orders = response.get('content', [])
                    if not orders:
                        self.logger.info(f"No more {status} orders on page {page}")
                        break
                    ...
                    if page >= total_pages - 1:
                        self.logger.info(f"Reached last page for {status} orders")
                        break
                    
                    page += 1
                
                all_orders.extend(status_orders)
                self.logger.info(f"Fetched {len(status_orders)} total {status} orders across {page + 1} page(s)")
